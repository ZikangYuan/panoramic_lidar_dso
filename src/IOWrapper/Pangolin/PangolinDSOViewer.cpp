...
PangolinDSOViewer::PangolinDSOViewer(int w, int h, bool startRunThread)
{
...

	{
		...
		// 2021.11.23
		for(int i=0;i<5;i++){
			internalVideoImg[i] = new MinimalImageB3(h,w);
			internalKFImg[i] = new MinimalImageB3(h,w);
		}
		// 2021.11.23
		internalResImg = new MinimalImageB3(h,w);
		videoImgChanged=kfImgChanged=resImgChanged=true;

		// 2021.11.23
		for(int i=0;i<5;i++){
			internalVideoImg[i]->setBlack();
			internalKFImg[i]->setBlack();
		}
		// 2021.11.23
...
void PangolinDSOViewer::run()
{
	...
	pangolin::View& d_video = pangolin::Display("imgVideo")
		.SetAspect(h/(float)w);
	pangolin::View& d_kfDepth1 = pangolin::Display("imgKFDepth1")
		.SetAspect(h/(float)w);
	pangolin::View& d_video1 = pangolin::Display("imgVideo1")
		.SetAspect(h/(float)w);
	pangolin::View& d_kfDepth2 = pangolin::Display("imgKFDepth2")
		.SetAspect(h/(float)w);
	pangolin::View& d_video2 = pangolin::Display("imgVideo2")
		.SetAspect(h/(float)w);
	pangolin::View& d_kfDepth3 = pangolin::Display("imgKFDepth3")
		.SetAspect(h/(float)w);
	pangolin::View& d_video3 = pangolin::Display("imgVideo3")
		.SetAspect(h/(float)w);
	pangolin::View& d_kfDepth4 = pangolin::Display("imgKFDepth4")
		.SetAspect(h/(float)w);
	pangolin::View& d_video4 = pangolin::Display("imgVideo4")
		.SetAspect(h/(float)w);

	pangolin::View& d_residual = pangolin::Display("imgResidual")
		.SetAspect(h/(float)w);

	pangolin::GlTexture texKFDepth(h,w,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
	pangolin::GlTexture texVideo(h,w,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
	pangolin::GlTexture texKFDepth1(h,w,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
	pangolin::GlTexture texVideo1(h,w,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
	pangolin::GlTexture texKFDepth2(h,w,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
	pangolin::GlTexture texVideo2(h,w,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
	pangolin::GlTexture texKFDepth3(h,w,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
	pangolin::GlTexture texVideo3(h,w,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
	pangolin::GlTexture texKFDepth4(h,w,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
	pangolin::GlTexture texVideo4(h,w,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
	// 2021.11.23

	pangolin::GlTexture texResidual(h,w,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
    ...
    pangolin::CreateDisplay()
		  ...
		  .AddDisplay(d_video)
		  .AddDisplay(d_kfDepth1)
		  .AddDisplay(d_video1)
		  .AddDisplay(d_kfDepth2)
		  .AddDisplay(d_video2)
		  .AddDisplay(d_kfDepth3)
		  .AddDisplay(d_video3)
		  .AddDisplay(d_kfDepth4)
		  .AddDisplay(d_video4)
	...
	// xiang.2022.1.13
	pangolin::Var<bool> settings_showgt("ui.GT",true,true);
	// xiang.2022.1.13
	...
	while( !pangolin::ShouldQuit() && running )
	{
		...
		// 2021.11.23
		if(videoImgChanged){
			texVideo.Upload(internalVideoImg[0]->data,GL_BGR,GL_UNSIGNED_BYTE);
			texVideo1.Upload(internalVideoImg[1]->data,GL_BGR,GL_UNSIGNED_BYTE);
			texVideo2.Upload(internalVideoImg[2]->data,GL_BGR,GL_UNSIGNED_BYTE);
			texVideo3.Upload(internalVideoImg[3]->data,GL_BGR,GL_UNSIGNED_BYTE);
			texVideo4.Upload(internalVideoImg[4]->data,GL_BGR,GL_UNSIGNED_BYTE);
		}
		if(kfImgChanged){
			texKFDepth.Upload(internalKFImg[0]->data,GL_BGR,GL_UNSIGNED_BYTE);
			texKFDepth1.Upload(internalKFImg[1]->data,GL_BGR,GL_UNSIGNED_BYTE);
			texKFDepth2.Upload(internalKFImg[2]->data,GL_BGR,GL_UNSIGNED_BYTE);
			texKFDepth3.Upload(internalKFImg[3]->data,GL_BGR,GL_UNSIGNED_BYTE);
			texKFDepth4.Upload(internalKFImg[4]->data,GL_BGR,GL_UNSIGNED_BYTE);
		}
		...
		if(setting_render_displayVideo)
		{
			...
			d_video1.Activate();
			glColor4f(1.0f,1.0f,1.0f,1.0f);
			texVideo1.RenderToViewportFlipY();
			
			d_video2.Activate();
			glColor4f(1.0f,1.0f,1.0f,1.0f);
			texVideo2.RenderToViewportFlipY();

			d_video3.Activate();
			glColor4f(1.0f,1.0f,1.0f,1.0f);
			texVideo3.RenderToViewportFlipY();

			d_video4.Activate();
			glColor4f(1.0f,1.0f,1.0f,1.0f);
			texVideo4.RenderToViewportFlipY();
			// 2021.11.23
		}
		...
		{	
			...
			d_kfDepth1.Activate();
			glColor4f(1.0f,1.0f,1.0f,1.0f);
			texKFDepth1.RenderToViewportFlipY();
			d_kfDepth2.Activate();
			glColor4f(1.0f,1.0f,1.0f,1.0f);
			texKFDepth2.RenderToViewportFlipY();
			d_kfDepth3.Activate();
			glColor4f(1.0f,1.0f,1.0f,1.0f);
			texKFDepth3.RenderToViewportFlipY();
			d_kfDepth4.Activate();
			glColor4f(1.0f,1.0f,1.0f,1.0f);
			texKFDepth4.RenderToViewportFlipY();
			// 2021.11.23
		}
		...
		// xiang.2022.1.13
		this->settings_showgt = settings_showgt.Get();
		// xiang.2022.1.13
...
void PangolinDSOViewer::drawConstraints()
{
	...
	if(settings_showgt)
	{
		float colorRed[3] = {0,1,0};
		glColor3f(colorRed[0],colorRed[1],colorRed[2]);
		glLineWidth(3);
		glBegin(GL_LINE_STRIP);
		for(unsigned int i=0;i<keyframes.size();i++)
		{
			glVertex3f((float)keyframes[i]->camToWorld_gt.translation()[0],
					(float)keyframes[i]->camToWorld_gt.translation()[1],
					(float)keyframes[i]->camToWorld_gt.translation()[2]);		
		}
		glEnd();
	}
	// xiang.2022.1.13
	...
	for(int x=0;x<h;x++){
		for(int y=0;y<w;y++){
			...
			for(int i=0;i<cam_num;i++){
				int c = image->frame[i]->dI[x*w+y][0]*0.8 > 255.0f ? 255.0 : image->frame[i]->dI[x*w+y][0]*0.8;
				internalVideoImg[i]->at(x,y)=Vec3b(c,c,c);
			}
			// 2021.11.23
		}
	}
...
void PangolinDSOViewer::pushDepthImage(std::vector<MinimalImageB3*>& image)
{
	...
	// 2021.11.23
	for(int i=0;i<cam_num;i++){
		memcpy(internalKFImg[i]->data, image[i]->data, w*h*3);
	}
	// 2021.11.23
	kfImgChanged=true;
}
