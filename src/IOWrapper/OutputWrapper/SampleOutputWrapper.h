...
public:
        ...
        virtual void publishKeyframes( std::vector<FrameHessian*> &frames, bool final, CalibHessian* HCalib) override
        {
            for(FrameHessian* f : frames)
            {
                printf("OUT: KF %d (%s) (id %d, tme %f): %d active, %d marginalized, %d immature points. CameraToWorld:\n",
                       ...
                       f->shell->incoming_id,
                       f->shell->timestamp,
                       (int)f->frame[0]->pointHessians.size(), (int)f->frame[0]->pointHessiansMarginalized.size(), (int)f->frame[0]->immaturePoints.size());// 2022.1.11
                std::cout << f->shell->camToWorld.matrix3x4() << "\n";


                int maxWrite = 5;
                for(PointHessian* p : f->frame[0]->pointHessians)// 2022.1.11
                {
        ...
        virtual void pushDepthImage(std::vector<MinimalImageB3*>& image) override
        {
