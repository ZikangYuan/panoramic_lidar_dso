...
struct RawResidualJacobian
{
	...
	// XTL：Jpdxi为投影点坐标对host到target位姿的导数 the two rows of d[x,y]/d[xi]. 2x6
	Vec6f Jpdxi[2];			

	// the two rows of d[x,y]/d[C].
	VecCf Jpdc[2];			

	// the two rows of d[x,y]/d[E].
	Vec6f Jpde[2];			

	// the two rows of d[x,y]/d[idepth].
	Vec2f Jpdd;				

	// XTL：Jpdxi为误差对投影点的导数 the two columns of d[r]/d[x,y].9x2
	VecNRf JIdx[2];			

	// XTL：Jpdxi为误差对光度参数的导数 = the two columns of d[r] / d[ab]
	VecNRf JabF[2];			
