...
public:
  ...
  inline void initialize()
  {
    ...
    num = numIn1 = numIn1k = numIn1m = 0;
	// 2021.12.20
	std::cout<<"Accumulator14 initializing!"<<std::endl;
  }
...
public:
  ...
  inline void initialize()
  {
	...
	H.setZero();
	b.setZero();
    memset(SSEData,0, sizeof(float)*4*dim1/*45*/);  // 会对128位, 16字节进行对齐, 因此每个数用4个float存
    memset(SSEData1k,0, sizeof(float)*4*dim1);
    memset(SSEData1m,0, sizeof(float)*4*dim1);
    num = numIn1 = numIn1k = numIn1m = 0;
  }
  ...
  inline void finish()
  {
	...
	for(int r=0;r<7;r++)
	  ...
	  assert(idx==4*dim1/*45*/);
  }

// 计算一个dim维向量相乘, 得到dim*dim矩阵
  inline void updateSSE(const __m128 J[7])
  {
		// 一共45个值
	  float* pt=SSEData;
	  for(int i=0;i<7;i++){
		  for(int j=i;j<7;j++){
		 	 _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(J[i],J[j]))); pt+=4;
		  }
	  }
	  num+=4;
	  numIn1++;  // 乘一次加一
...
// 带权重的9维向量得到9*9矩阵
		  ...
		  const __m128 J[8])
  {
	  ...
	  for(int i=0;i<7;i++)
	  {
		  __m128 Jw = _mm_mul_ps(J[i],J[7]);
		  for(int j=i;j<7;j++){
	  		_mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(Jw,J[j]))); pt+=4;
		  }
	  }
...
// 不使用_m128来计算
  inline void updateSingle(const float J[7], int off=0)
  {
	  float* pt=SSEData+off;
	  for(int i=0;i<7;i++){
		  for(int j=i;j<7;j++){
...
// 不使用对齐加速的, 带有权重的
  inline void updateSingleWeighted(float J[8], int off=0)
  {
...
private:
  ...
  void shiftUp(bool force)
  {
	  if(numIn1 > 1000 || force)
	  {
		  for(int i=0;i<dim1/*45*/;i++)
			  _mm_store_ps(SSEData1k+4*i, _mm_add_ps(_mm_load_ps(SSEData+4*i),_mm_load_ps(SSEData1k+4*i)));
		  numIn1k+=numIn1;
		  numIn1=0;
		  memset(SSEData,0, sizeof(float)*4*dim1);
	  }
...
public:
  ...
  int dim;
  int dim1;
  Eigen::Matrix<float,2*CAM_NUM+7,2*CAM_NUM+7> H;
  Eigen::Matrix<float,2*CAM_NUM+7,1> b;
...
// 计算一个dim维向量相乘, 得到dim*dim矩阵
  inline void updateSSE(const __m128 J[2*CAM_NUM+7])
  {
		// 一共45个值
	  float* pt=SSEData;
	  // 2021.12.14
...
// 带权重的9维向量得到9*9矩阵
  inline void updateSSE_eighted(
		  const __m128 J[2*CAM_NUM+8])
  {
	  float* pt=SSEData;
	  for(int i=0;i<dim;i++)
	  {
		  __m128 Jw = _mm_mul_ps(J[i],J[dim]);
		  for(int j=i;j<dim;j++){
	  		_mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(Jw,J[j]))); pt+=4;
		  }
	  }
...
// 不使用_m128来计算
  inline void updateSingle(const float J[2*CAM_NUM+7], int off=0)
  {
	  float* pt=SSEData+off;
	  // 2021.12.14
	  for(int i=0;i<dim;i++){
...
// 不使用对齐加速的, 带有权重的
  inline void updateSingleWeighted(float J[2*CAM_NUM+8], int off=0)
  {
...
private:
  EIGEN_ALIGN16 float SSEData[4*(2*CAM_NUM+7)*(CAM_NUM+4)];
  EIGEN_ALIGN16 float SSEData1k[4*(2*CAM_NUM+7)*(CAM_NUM+4)];
  EIGEN_ALIGN16 float SSEData1m[4*(2*CAM_NUM+7)*(CAM_NUM+4)];
  float numIn1, numIn1k, numIn1m;
