...
void EnergyFunctional::setAdjointsF(CalibHessian* Hcalib)
{
	...
	for(int h=0;h<nFrames;h++) // 主帧
		...
		{
			FrameHessian* host = frames[h]->data;	//FrameHessian信息
			FrameHessian* target = frames[t]->data;
			
			//TODO !!! 2022.1.5
			// XTL：这里的get_worldToCam_evalPT返回worldToCam_evalPT，是world到cam的估计的位姿变换关系
			SE3 hostToTarget = target->get_worldToCam_evalPT() * host->get_worldToCam_evalPT().inverse();	//线性化点的信息
			...
			Vec2f affLL = AffLight::fromToVecExposure(host->ab_exposure[0], target->ab_exposure[0], host->aff_g2l_0(0), target->aff_g2l_0(0)).cast<float>();
			AT(6,6) = -affLL[0]; //! a'(aj)
			AH(6,6) = affLL[0];	 //! a'(ai)
			AT(7,7) = -1;		 //! b'(bj)
			AH(7,7) = affLL[0];	 //! b'(bi)
			...
			// XTL：adHost是host到target的位姿对world到host位姿求导的结果
			// XTL：adTarget是host到target的位姿对world到target位姿求导的结果
...
//@ 计算各种状态的相对量的增量
// XTL：该函数只在setPrecalcValues调用，而setPrecalcValues只在创建关键帧后以及优化迭代中调用
// XTL：这里改变了efframe的delta与delta_prior，优化迭代中调用时可以及时地更新这两个变量
// XTL：delta_prior会在优化过程中与prior相乘
void EnergyFunctional::setDeltaF(CalibHessian* HCalib)
{
	// XTL：这里的frames->data目前只可能是主帧，get_state_minus_stateZero返回的是进一步优化时状态的增量
	if(adHTdeltaF != 0) delete[] adHTdeltaF;
	...
	for(int h=0;h<nFrames;h++)
		...
		{
			int idx = h+t*nFrames;
			// XTL：adHTdeltaF为host到target的位姿的增量
	...
	// TODO
	cDeltaF = HCalib->value_minus_value_zero[0].cast<float>(); // 相机内参增量

	
	for(EFFrame* f : frames)
	{
		f->delta = f->data->get_state_minus_stateZero().head<8>();  
		f->delta_prior = (f->data->get_state() - f->data->getPriorZero()).head<8>(); // XTL 帧位姿以及光度增量
		for(EFPoint* p : f->points)
			p->deltaF = p->data->idepth - p->data->idepth_zero; // 逆深度的增量
		//std::cout << " f->delta->prior: " << f->data->getPriorZero().head<6>().matrix() << std::endl;
	}
...
void EnergyFunctional::accumulateAF_MT(MatXX &H, VecX &b, bool MT)
{
	...
	else
	{
		...
		accSSE_top_A->stitchDoubleMT(red,H,b,this,false,false); // 不加先验
		// accSSE_top_A->stitchDoubleMT(red,H,b,this,true,false); // 加先验
...
void EnergyFunctional::resubstituteF_MT(VecX x, CalibHessian* HCalib, bool MT)
{
	...
	for(int i=0;i<cam_num;i++){
		//HCalib->step[i] = - x.head<4>(4*i);
		HCalib->step[i].setZero();
	}

	Mat18f* xAd = new Mat18f[nFrames*nFrames];

	VecCf cstep;/* = xF.head<CPARS>();*/
	//std::cout<<"The intrinsic change is:"<<xF.head<CPARS>()<<std::endl;// 2022.1.13
	cstep.setZero();

	for(EFFrame* h : frames)
	{
		h->data->step.head<8>() = - x.segment<8>(CPARS+8*h->idx); // 帧位姿和光度求解的增量
		h->data->step.tail<4>().setZero(); // 2022.11.22 fixed aff

		//* 绝对位姿增量变相对的
		// XTL：xAd = -deltaT(h)^T*(dT(th)/dT(h))-deltaT(h)^T*(dT(th)^T/dT(t))
		// XTL：    = -deltaT(th)^T
        ...
        const VecCf &xc, Mat18f* xAd, int min, int max, Vec10* stats, int tid)
{
	for(int k=min;k<max;k++)
	{
		...
		// bdSumF:(dr/didepth)^T*r
		float b = p->bdSumF;
		// XTL   Hcd_accXF：(dr/dC)^T*(dr/didepth) b+=(dr/didepth)^T*r
		b -= xc.dot(p->Hcd_accAF + p->Hcd_accLF);

		for(EFResidual* r : p->residualsAll)
		{
			if(!r->isActive()) continue;
			// XTL   b+=deltaT(th)^T*(dr/dT(th))^T*(dr/didepth)
			b -= xAd[r->hostIDX*nFrames + r->targetIDX] * r->JpJdF;
		}

		if(p->data->isFromSensor==true && !setting_optDepth)
			p->data->step = 0; // 2020.09.28 yzk shiyong
		else
			p->data->step = - b*p->HdiF; // 逆深度的增量 2020.09.28 yzk zhushi
	}
}
...
// XTL setting_forceAceptStep为true时，不进入该函数
...
double EnergyFunctional::calcLEnergyF_MT()
{
	...
	for(EFFrame* f : frames)
        E += f->delta_prior.cwiseProduct(f->prior).dot(f->delta_prior); // 位姿先验 cwiseProduct()函数允许Matrix直接进行点对点乘法，而不用转换至Array。

	E += cDeltaF.cwiseProduct(cPriorF).dot(cDeltaF); // 相机内参先验

	red->reduce(boost::bind(&EnergyFunctional::calcLEnergyPt,
			this, _1, _2, _3, _4), 0, allPoints.size(), 50);	//red应该是加速计算用的
...
EFResidual* EnergyFunctional::insertResidual(PointFrameResidual* r)
{
	...
	efr->idxInAll = r->point->efPoint->residualsAll.size(); // 在这个点的所有残差中的id
	r->point->efPoint->residualsAll.push_back(efr); // 插入到这个点的所有残差中
...
EFFrame* EnergyFunctional::insertFrame(FrameHessian* fh, CalibHessian* Hcalib)
{
	...
	for(int i=0;i<cam_num;i++)
		fh->frame[i]->efFrame = eff; // FrameHessian 指向能量函数帧

	//由于滑窗内的帧最多为7个 所以HM的列数最多为60
	assert(HM.cols() == 8*nFrames+CPARS-8);  // 边缘化掉一帧, 缺8个

	// 一个帧8个参数 + 相机内参
	bM.conservativeResize(8*nFrames+CPARS); //conservativeResize改变尺寸的同时 不改变其中的内容
...
// XTL：把该残差从主点所有的残差项中删去，然后在connectivityMap中减少主帧和目标帧之间的连接
...
//[ ***step 1*** ] 把边缘化的帧挪到最右边, 最下边
	...
	if((int)fh->idx != (int)frames.size()-1)
	{
		...
		Vec8 bTmp = bM.segment<8>(io); // 被边缘化的VAR_NUM个变量 2021.12.19
...
//[ ***step 2*** ] 加上先验
	...
	// XTL：cwiseProduct为H积，即对应位置的元素相乘
...
//[ ***step 3*** ] 先scaled 然后计算Schur complement
	...
	// schur-complement!XTL:参考slam十四讲P253的公式
...
//@ 边缘化掉一个点
// 遍历所有efframe上的EFPoint，若其状态为边缘化的，则priorF乘上setting_idepthFixPriorMargFac，把点放入allPointsToMarg，对于该点所有的残差项，若残差是IN，则在connectivityMap中增加连接
...
//[ ***step 1*** ] 记录被边缘化的点
	...
	for(EFFrame* f : frames)
	{
		for(int i=0;i<(int)f->points.size();i++)
		{
			...
			{
				p->priorF *= setting_idepthFixPriorMargFac; //? 这是干啥 ??? //现在的框架中p应该是没有先验的
				for(EFResidual* r : p->residualsAll)
...
// XTL：遍历所有EFFrame的EFPoint，若其状态为PS_DROP，则调用removePoint
...
// XTL：遍历该EFPoint所有的efresidual，把它们丢掉，然后从EFFrame的points中删去该EFPoint
...
void EnergyFunctional::solveSystemF(int iteration, double lambda, CalibHessian* HCalib)
{
	...
	// XTL：遍历所有激活点，将其导数放入HA_top((4+num×8)×(4+num×8))、bA_top((4+num×8)×1)矩阵
	...
	// bM可以看作对运动、光度的一个约束，大部分情况下起约束光度的作用，第一帧会约束运动
	...
	if(setting_solverMode & SOLVER_ORTHOGONALIZE_SYSTEM)
	{
		...
		if(!haveFirstFrame)
			orthogonalize(&bT_act, &HT_act); // 5相机需要零空间?
