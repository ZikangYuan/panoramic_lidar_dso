...
// XTL:对于该点的所有残差项，计算了对应的两帧之间的导数，一部分放入acc，一部分放入该点的Hdd_accAF等
template<int mode>
void AccumulatedTopHessianSSE::addPoint(EFPoint* p, EnergyFunctional const * const ef, int tid)	// 0 = active, 1 = linearized, 2=marginalize 这里的tid是这是为0的 纯单线程
{

	assert(mode==0 || mode==1 || mode==2);

	VecCf dc = ef->cDeltaF;
	float dd = p->deltaF;	// 逆深度	//这里固定为0
	...
	for(EFResidual* r : p->residualsAll)
	{
		...
		// XTL：resApprox为点的总残差
		VecNRf resApprox;
		// XTL：resF是点的残差，res_toZeroF可以看作已经边缘化的点，边缘化时由于位姿造成残差项的改变量加上原来的残差
		if(mode==0)
			resApprox = rJ->resF;
		if(mode==2) // 边缘化时使用的
		...
		{
			// compute Jp*delta
			// XTL：Jpdxi为投影点坐标对host到target位姿的导数
		...
		// XTL:  Jpdc:(dp/dC) Jpdxi:(dp/dT) JabJIdx:(dr/dab)^T*(dr/dp) JI_r:(dr/dp)^T*r Jab_r:(dr/dab)^T*r JIdx2: (dr/dp)^T*(dr/dp)
		// XTL:[(dr/dC)^T*(dr/dC) (dr/dC)^T*(dr/dT)]
		// 	   [(dr/dT)^T*(dr/dC) (dr/dT)^T*(dr/dT)]
		...
		// XTL:[(dr/dab)^T*(dr/dab) (dr/dab)^T*r]
		// 	   [     r^T*(dr/dab)       r^T*r   ]
		// (2*cam_num+1)*(2*cam_num+1)
		acc[tid][htIDX].updateBotRight(
				rJ->Jab2(0,0), rJ->Jab2(0,1), Jab_r[0],
				rJ->Jab2(1,1), Jab_r[1],rr);

		
		// XTL:[(dr/dC)^T*(dr/dab)   (dr/dC)^T*r]
		// 	   [(dr/dT)^T*(dr/dab)   (dr/dT)^T*r]
		...
		// XTL：(dr/dp)^T*(dr/didepth)
		Vec2f Ji2_Jpdd = rJ->JIdx2 * rJ->Jpdd;
		// XTL：(dp/didepth)^T*(dr/dp)^T*r = (dr/didepth)^T*r
		bd_acc +=  JI_r[0]*rJ->Jpdd[0] + JI_r[1]*rJ->Jpdd[1];
		// XTL：(dp/didepth)^T*(dr/dp)^T*(dr/didepth) = (dr/didepth)^T*(dr/didepth)
		Hdd_acc += Ji2_Jpdd.dot(rJ->Jpdd);
		// XTL：(dr/dC)^T*(dr/didepth)
...
void AccumulatedTopHessianSSE::stitchDouble(MatXX &H, VecX &b, EnergyFunctional const * const EF, bool usePrior, bool useDelta, int tid)
{
	...
	for(int h=0;h<nframes[tid];h++)
		for(int t=0;t<nframes[tid];t++)
		{
			...
			MatPCPC accH = acc[tid][aidx].H.cast<double>();	//将之前计算的acc的数值取出来
	...
	// ----- new: copy transposed parts. 由H矩阵的对称性得到另一半的数值
	for(int h=0;h<nframes[tid];h++)
	{
		int hIdx = CPARS+h*8;
	...
	if(usePrior)
	{
		...
		for(int h=0;h<nframes[tid];h++)
		{
            ...
            b.segment<8>(CPARS+h*8) += EF->frames[h]->prior.cwiseProduct(EF->frames[h]->delta_prior);	//cwiseProduct代表点乘
		}
	}
}

// XTL usePrior默认为false，
		...
		int min, int max, Vec10* stats, int tid)
{
	...
	for(int k=min;k<max;k++) // 帧的范围 最大nframes[0]*nframes[0]
	{
		...
		MatPCPC accH = MatPCPC::Zero(); // (8+4+1)*(8+4+1) //1保存残差值
		...
		//* 相对的量通过adj变成绝对的量, 并累加到 H, b 中	noalias解决混淆问题
	...
	if(min==0 && usePrior)
	{
		...
		for(int h=0;h<nframes[tid];h++)
		{
			...
			// std::cout<<"H is:\n"<<H[tid].block<8,8>(CPARS+h*8,CPARS+h*8)<<std::endl;
			// std::cout<<"b is:\n"<<b[tid].segment<8>(CPARS+h*8).transpose()<<std::endl;
		}
	}
}
