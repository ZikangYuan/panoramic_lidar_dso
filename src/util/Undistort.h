...
public:
	...
	// 2019.11.07 yzk
	template<typename T>
	ImageAndExposure* transformDepthImage(const MinimalImage<T>* image_raw, float exposure=0, double timestamp=0, float factor=1) const;
	// 2019.11.07 yzk
	static Undistort* getUndistorterForFile(std::string configFilename, std::string gammaFilename, std::string vignetteFilename, std::string undistortFile);
...
protected:
	...
	void readFromFile(const char* configFileName, int nPars, std::string prefix = "", std::string undistortFile = "");
};
...
// 2021.10.28
class UndistortOmni : public Undistort
{
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    UndistortOmni(const char* configFileName, bool noprefix, std::string undistortFile = "");
	~UndistortOmni();
	void distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y, int n) const;
};
// 2021.10.28
