...
Undistort* Undistort::getUndistorterForFile(std::string configFilename, std::string gammaFilename, std::string vignetteFilename, std::string undistortFile)
{
	...
	{
		f.close();
		printf("%s not found. Cannot operate without calibration, shutting down.\n",configFilename.c_str());
		f.close();
		return 0;
	}
	...
	// 2021.10.22
    else if(std::sscanf(l1.c_str(), "Omni %f %f %f %f",
            &ic[0], &ic[1], &ic[2], &ic[3]) == 4)
    {
		u = new UndistortOmni(configFilename.c_str(), false, undistortFile.c_str());
		if(!u->isValid()) {delete u; return 0; }
    }
	// 2021.10.22
...
//* 
void Undistort::readFromFile(const char* configFileName, int nPars, std::string prefix, std::string undistortFile)
{
	photometricUndist=0;
	valid = false;
	passthrough=false;
...
//* 第一行, 相机模型参数; 第二行, 相机像素大小
	...
	// 2021.10.22
	else if(nPars == 4) // KB, equi radtan &omni model
	{
		char buf[1000];
		snprintf(buf, 1000, "%s%%lf %%lf %%lf %%lf", prefix.c_str());

		if(std::sscanf(l1.c_str(), buf, &parsOrg[0], &parsOrg[1], &parsOrg[2], &parsOrg[3]) == 4 &&
				std::sscanf(l2.c_str(), "%d %d", &wOrg, &hOrg) == 2)
		{
			printf("Input resolution: %d %d\n",wOrg, hOrg);
			printf("In: %s%f %f %f %f\n",
...
//* 第三行, 相机图像类别, 是否裁切
	...
	{
		outputCalibration[0] = -4;
        printf("Out: Omni Rectification\n");
	}
	// 2021.10.27 
	// 2022.1.2
	else if(l3 == "Omni_corrected")
	{
		outputCalibration[0] = -5;
        printf("Out: Omni_corrected Rectification\n");
	}
	// 2022.1.2
	else
	{
...
//* 得到合适的相机参数
	...
	else if(outputCalibration[0] == -4)
	{
		...
		// 2021.10.28
		mask = new bool[w*h];
		memset(mask,true,sizeof(bool)*w*h);
		// 2021.10.28

		// printf("Reading Undistort from file %s",undistortFile.c_str());

		std::ifstream infile1(undistortFile.c_str());
		if (!infile1.good())
		{
			infile1.close();
			printf("%s not found. Cannot operate without Undistort, shutting down.\n",undistortFile.c_str());
			infile1.close();
			return ;
		}
		...
		for(int y=0;y<h;y++)
			for(int x=0;x<w;x++)
			{
				...
				else
				{
					...
					// 2021.11.25
					/*
					if(x+setting_margin>_left&&x<w/2) _left = x+setting_margin;
					if(x-setting_margin<_right&&x>w/2) _right = x-setting_margin;
					if(y+setting_margin>_top&&y<h/2) _top = y+setting_margin;
					if(y-setting_margin<_bottom&&y>h/2) _bottom = y-setting_margin;
					*/
					// 2021.11.25
...
//********************* 以下都是加畸变的算法 *******************************
// 2021.10.22
UndistortOmni::UndistortOmni(const char* configFileName, bool noprefix, std::string undistortFile)
{
    printf("Creating Omni undistorter\n");

    if(noprefix)
        readFromFile(configFileName, 4);
    else
        readFromFile(configFileName, 4, "Omni ", undistortFile);
}
UndistortOmni::~UndistortOmni()
{
}
void UndistortOmni::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y, int n) const{}
// 2021.10.22
