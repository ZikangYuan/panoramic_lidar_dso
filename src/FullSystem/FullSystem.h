...
public:
	...
	void addActiveFrame(std::vector<ImageAndExposure*> image, int id);
    ...
    std::queue<cv::Mat> qImg[CAM_NUM];

	int frameNum;
	int havePushed;
	int cnt;
	bool haveMakedKF;
    std::queue<std::vector<Eigen::Vector4d,Eigen::aligned_allocator<Eigen::Vector4d>>> Clouds;
    std::queue<std::vector<uchar>> Intensities;
    short* intensity;
    std::queue<std::vector<Eigen::Vector3d,Eigen::aligned_allocator<Eigen::Vector3d>>> Cloud_ijrr;
    std::queue<std::vector<Eigen::Vector3d,Eigen::aligned_allocator<Eigen::Vector3d>>> cloudsWindow;
    std::queue<std::vector<Eigen::Vector3d,Eigen::aligned_allocator<Eigen::Vector3d>>> cloudsWindow_ijrr[CAM_NUM];

    std::vector<Eigen::Vector4d,Eigen::aligned_allocator<Eigen::Vector4d>> cloudPixel[CAM_NUM];
	std::vector<Eigen::Vector4d,Eigen::aligned_allocator<Eigen::Vector4d>> bestCloud[CAM_NUM];

	std::vector<int> idxUseLast;
	...
	void drawPointDistribution(int idx, FrameHessian* fh, std::vector<Eigen::Vector3d,Eigen::aligned_allocator<Eigen::Vector3d>> Clouds);
    void loadSensorPrameters(const std::string &pathSensorParameter);
	void loadSensorPrameters_ijrr(const std::string &pathSensorParameter);
	void setKeyFrameWeight(FrameHessian* fh);
	float calcRes(Mat66& H, Vec6& b, SE3 T_lb3_lidar, int lvl,
	std::vector<Eigen::Vector4d,Eigen::aligned_allocator<Eigen::Vector4d>> cld, CalibHessian* HCalib);
	void optExtrinsic(SE3 &T_lb3_lidar);
	double ResPixel(
		const float outlierTHSlack,int idx, FrameHessian* target, float u, float v,
		cv::Mat &Hdd, cv::Mat &bd, int level,float mean_diff,float idepth);
	bool ProjectCloud(std::vector<Eigen::Vector4d,Eigen::aligned_allocator<Eigen::Vector4d>>& cld);
	void ProjectCloud(std::vector<Eigen::Vector4d,Eigen::aligned_allocator<Eigen::Vector4d>>& cld, 
		SE3 T_lb3_lidar,bool debug=false,float res=0,bool before=false);
	bool ProjectCloud_ijrr(std::vector<Eigen::Vector3d,Eigen::aligned_allocator<Eigen::Vector3d>> cld[CAM_NUM]);

	// 2020.07.03 yzk shiyong
	CalibHessian Hcalib;
	
	// lidar to cam
	Eigen::Matrix3d Rcl[CAM_NUM];
	Eigen::Vector3d tcl[CAM_NUM];

	// lidar to lb3
	float x_lidar_lb3_ori[6];
	...
	void loadTimeStamp(std::vector<std::string> timeLidar);
	void loadTimeStamp_ijrr(std::string time_stamp_path);
	std::vector<std::string> time_lidar;
...
private:
	...
	Vec4 trackNewCoarse_quick(FrameHessian* fh);
	void traceNewCoarse(FrameHessian* fh);
	bool trackNewToSelectKF();
	void OptExtrinsic(std::vector<uchar>& intensity_,FrameHessian* fh,
	std::vector<Eigen::Vector3d,Eigen::aligned_allocator<Eigen::Vector3d>>& cld);
	bool OptExtrinsicByImageAlign(FrameHessian* fh, SE3& T_lb3_lidar, Vec5 &res);
	void activatePoints();
	...
	std::queue<FrameHessian*> fhsWindow;	
 ...
 */
	...
	void makeFrame( FrameHessian* fh);
