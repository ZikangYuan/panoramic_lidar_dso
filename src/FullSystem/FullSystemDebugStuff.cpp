...
namespace dso
{
	...
	void FullSystem::debugPlotTracking()
	{
		...
		for(FrameHessian* f : frameHessians)
		{
			...
			for(FrameHessian* f2 : frameHessians)
				if(f2->frame[0]->debugImage == 0) f2->frame[0]->debugImage = new MinimalImageB3(wG[0], hG[0]); // 2022.1.11

			for(FrameHessian* f2 : frameHessians)
			{
				MinimalImageB3* debugImage=f2->frame[0]->debugImage;// 2022.1.11
				images.push_back(debugImage);

				Eigen::Vector3f* fd = f2->frame[0]->dI;

				Vec2 affL = AffLight::fromToVecExposure(f2->ab_exposure[0], f->ab_exposure[0], f2->aff_g2l(0), f->aff_g2l(0)); // 2021.12.15 TODO
			...
			for(PointHessian* ph : f->frame[0]->pointHessians)
			{
				...
				if(ph->status == PointHessian::ACTIVE || ph->status == PointHessian::MARGINALIZED)
				{
					for(PointFrameResidual* r : ph->residuals)
						r->debugPlot();
					f->frame[0]->debugImage->setPixel9(ph->u+0.5, ph->v+0.5, makeRainbow3B(ph->idepth_scaled));
				}
			}
	...
	void FullSystem::debugPlot(std::string name)
	{
		...
		if((int)(freeDebugParam5+0.5f) == 7 || (debugSaveImages&&false))
		{
			...
			{
				for(PointHessian* ph : frameHessians[f]->frame[0]->pointHessians)// 2022.1.11
					if(ph!=0) allID.push_back(ph->idepth_scaled);

				for(PointHessian* ph : frameHessians[f]->frame[0]->pointHessiansMarginalized)
					if(ph!=0) allID.push_back(ph->idepth_scaled);

				for(PointHessian* ph : frameHessians[f]->frame[0]->pointHessiansOut)
					if(ph!=0) allID.push_back(ph->idepth_scaled);
			}
		...
		for(unsigned int f=0;f<frameHessians.size();f++)
		{
			...
			Eigen::Vector3f* fd = frameHessians[f]->frame[0]->dI;//2022.1.11
			...
			{
				for(PointHessian* ph : frameHessians[f]->frame[0]->pointHessians)
				{
					if(ph==0) continue;
			...
			{
				for(ImmaturePoint* ph : frameHessians[f]->frame[0]->immaturePoints)
				{
					if(ph==0) continue;
		...
		if((debugSaveImages&&false))
		{
			...
			{
				MinimalImageB3* img = new MinimalImageB3(wG[0],hG[0]);
				Eigen::Vector3f* fd = frameHessians[f]->frame[0]->dI;
