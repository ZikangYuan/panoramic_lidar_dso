...
private:
	...
	void choose_point(int lvl,std::vector<std::queue<int>>& point_choose,std::vector<std::vector<int>>& index);
	Vec6 calcRes(int lvl, const SE3 &refToNew, const SE3 refToNews[], SE3 slastToLast, float cutoffTH);
    Vec6 calcRes_ransac(int lvl, const SE3 &refToNew, const SE3 refToNews[], SE3 slastToLast, float cutoffTH,std::vector<std::queue<int>> point_choose);
    void cal_InitPoint(int lvl, const SE3 &refToNew, const SE3 refToNews[],std::vector<std::vector<int>>& index);
    int cal_InPoint(const SE3 &refToNew, const SE3 refToNews[],int coarsestLvl);
	Vec6 calcResAndInpointnum(int lvl, const SE3 &refToNew, const SE3 refToNews[], SE3 slastToLast, float cutoffTH,int& In_num);
	void preCompute(int lvl);
	Vec6 calcResInv(int lvl, const SE3 &refToNew, float cutoffTH, Mat66f& H, Vec6f& res);
	void ProjectForVisualization(
		std::vector<Eigen::Vector3d,Eigen::aligned_allocator<Eigen::Vector3d>>& cld,
		SE3 T_lb3_lidar, float res, bool before);
	bool getCloseViewObs(const float& x, const float& y, const float& id, const SE3 refToNews[], int idx_ori, int lvl, int& min_idx);
	Vec3 calcExtrinRes(
		std::vector<Eigen::Vector3d,Eigen::aligned_allocator<Eigen::Vector3d>>& cld,
	...
	int buf_warped[CAM_NUM*CAM_NUM];
